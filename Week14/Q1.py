...
def check_api_info(response):
    ...
    if "Server" in headers:
        findings.append(f"Server verson exposed: {headers['Server']}")
    if "X-Powered-By" in headers:
        findings.append(f"Server verson exposed: {headers['X-Powered-By']}")
    if headers.get("Access-Control-Allow-Origin") == "*":
        findings.append(f"CROS: open to all origins")
    return findings
